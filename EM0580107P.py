...
def InsertToDataBase():
    ...
    for a in range(len(EM0580107PData)):
        #Resetting Variables
        columnMover = 6
        emptyDetected = 0

        #Getting The Row, Column Location Of SUPPLIER
        findSupplier = [(index, column) for index, row in EM0580107PData[a].iterrows() for column, value in row.items() if value == "SUPPLIER"]
        ...
        while True:
            try:
                ...
                supplierFiltered = EM0580107PData[a].iloc[
                    max(0, supplierRow[0] - 3):min(len(EM0580107PData[a]), supplierRow[0] + 10),
                    EM0580107PData[a].columns.get_loc(supplierColumn[0] + columnMover):EM0580107PData[a].columns.get_loc(supplierColumn[0]) + 5 + columnMover
                ]
                columnMover += 5
                ...
                Sql.InsertDataToEM0580107PInspectionTable(dataFrame)
