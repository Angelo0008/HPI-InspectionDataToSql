...
def CreateEM0580106PInspection():
    global create_cursor

    create_cursor.execute("""
        CREATE TABLE EM0580106P_Inspection(
...
def InsertDataToEM0580106PInspectionTable(df):
    global create_cursor, mariadb_connection
    ...
    # First, get all existing datetimes from the database
    create_cursor.execute("SELECT ID FROM em0580106p_inspection")
    existing_datetimes = [row[0] for row in create_cursor.fetchall()]
    ...
    # Create the SQL insert statement
    insert_query = """
        INSERT INTO em0580106p_inspection (
            ID,
            Lot_Number,
            Date,
